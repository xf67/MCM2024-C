...
data_all=pd.read_csv("performance4.csv")
...
for i in data["DATA_NUM"] :
    print(f"index:{i}")
    print(f"p_no:{data['point_no'][i]}")
    if int(data["point_no"][i])==1:
    # a new player
        print("new")
    ...
    if i==0 or data["p1_score"][i]==0 and data["p2_score"][i]==0:
        atart()
...
data_all.to_csv('data_w_mmtm4.csv', index=False)
